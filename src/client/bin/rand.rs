...
pub fn handle_rand(stream: &mut TcpStream, args: &Vec<String>) {
  ...
  match format {
    ...
    "u8" => {
      ...
      match range {
        // no range means default [0, 256)
        None => println!("{:?}",rand_out),
        // specified range [range.0, range.1)
        Some(range) => { 
          let result = extract_u8s(range.0, range.1, &rand_out);
          println!("{:?}", result);
        },
      };
    },
    "u16" => {
      ...
      match range {
        ...
        None => {
          ...
          let mut i = 0;
          while i < rand_out.len() - 1 {
            ret.push(LittleEndian::read_u16(&rand_out[i..i+2]));
            i += 2
          }
          println!("{:?}", ret);
        },
        // specified range [range.0, range.1)
        Some(range) => { 
          let result = extract_u16s(range.0, range.1, &rand_out);
          println!("{:?}", result);
        },
      };
    },
...
fn extract_u16s(a: u16, b:u16, r: &[u8]) -> Vec<u16> {
  let mut i: usize = 0;
  let mut ret: Vec<u16> = Vec::new();
  while i < r.len() - 1 {
    let result = unbiased_u16(a, b, LittleEndian::read_u16(&r[i..i+2]));
    match result {
      Some(x) => ret.push(x),
      None => (),
    }
